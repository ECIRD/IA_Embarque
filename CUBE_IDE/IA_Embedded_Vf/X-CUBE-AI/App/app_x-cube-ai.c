...
#if !defined(AI_MODEL_V1_INPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_in_1[AI_MODEL_V1_IN_1_SIZE_BYTES];
ai_i8* data_ins[AI_MODEL_V1_IN_NUM] = {
data_in_1
};
#else
ai_i8* data_ins[AI_MODEL_V1_IN_NUM] = {
...
#if !defined(AI_MODEL_V1_OUTPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_out_1[AI_MODEL_V1_OUT_1_SIZE_BYTES];
ai_i8* data_outs[AI_MODEL_V1_OUT_NUM] = {
data_out_1
};
#else
ai_i8* data_outs[AI_MODEL_V1_OUT_NUM] = {
...
AI_ALIGNED(32)
static uint8_t pool0[AI_MODEL_V1_DATA_ACTIVATION_1_SIZE];

ai_handle data_activations0[] = {pool0};

/* AI objects ----------------------------------------------------------------*/

static ai_handle model_v1 = AI_HANDLE_NULL;
...
static int ai_boostrap(ai_handle *act_addr)
{
  ...
  err = ai_model_v1_create_and_init(&model_v1, act_addr, NULL);
  if (err.type != AI_ERROR_NONE) {
    ai_log_err(err, "ai_model_v1_create_and_init");
    return -1;
  }

  ai_input = ai_model_v1_inputs_get(model_v1, NULL);
  ai_output = ai_model_v1_outputs_get(model_v1, NULL);
...
static int ai_run(void)
{
  ...
  batch = ai_model_v1_run(model_v1, ai_input, ai_output);
  if (batch != 1) {
    ai_log_err(ai_model_v1_get_error(model_v1),
        "ai_model_v1_run");
    return -1;
  }
